...
public class MainMenuLogic : Photon.MonoBehaviour {
    ...
    public virtual void OnReceivedRoomListUpdate() {
        if (GameState == States.JoinRoomScreen) {
            // Get existing room name buttons
            RoomNameHolder[] listedRooms = 
                JoinRoomScreen.GetComponentsInChildren<RoomNameHolder>();
            HashSet<string> listedNames = new HashSet<string>();
            for (int i = 0; i < listedRooms.Length; i++)
                listedNames.Add(listedRooms[i].roomName);

            // Compare existing rooms to available rooms
            RoomInfo[] currentRooms = PhotonNetwork.GetRoomList();
            foreach (RoomInfo room in currentRooms)
                if (listedNames.Contains(room.name))
            PopulateRoomList();
        }
    }
